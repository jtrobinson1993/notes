 ...
 *
 * - `message` / `reaction`: opens the conversation, adding the channel segment
 *   only when it differs from the conversation (the general channel is the bare
 *   path). A `?m=<seq>` scrolls to the message (for a reaction, the message that
 *   was reacted to); a `?thread=<parentSeq>` opens that thread panel (the
 *   message lives inside the thread).
