...
import { rememberRelayUrl, startRelayDelivery } from './nativeRelay';
...
export async function enrollThisDevice(): Promise<void> {
  ...
  // Remember the relay URL so a later cold start can redial (in the native
  // shell the app origin isn't the relay), then start live inbound delivery
  // (listen for `relay:mail` nudges + drain any backlog). Best-effort.
  await rememberRelayUrl(window.location.origin);
