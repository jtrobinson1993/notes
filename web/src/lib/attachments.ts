...
import { attachmentGet, attachmentPut, isNative } from './native';
...
// ---- note attachments ----
//
// Notes are local-only in the native shell (no relay sync yet), so their
// attachments never leave the device: the ciphertext goes straight into the
// vault's blob store instead of the legacy server's. In a browser the legacy
// upload/download endpoints are still the store. Callers stay identical, so
// notes behave the same either way.
...
export async function putNoteAttachment(
  ...
  if (!isNative) return (await api.attachmentUpload(ciphertext)).id;
...
export async function getNoteAttachmentCiphertext(id: string): Promise<Uint8Array | null> {
  if (!isNative) return api.attachmentDownload(id);
