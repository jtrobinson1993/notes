...
function conv(myPub: Uint8Array, sealedKey: Conversation['sealedKey'], over: Partial<Conversation> = {}): Conversation {
  return {
    ...
    channels: [], lastSeq: 0, lastReadSeq: 0, createdAt: 0, ...over,
  } as Conversation;
}
...
describe('loadConversations + loadHistory', () => {
  it('loads conversations, then decrypts fetched history into the store', async () => {
    ...
    expect(api.conversationMessages).toHaveBeenCalledWith('c1', { before: undefined, limit: 50, channelId: 'c1' });
  ...
  it('loadHistory passes the before cursor for older pages and returns the count', async () => {
    ...
    const store = useChatStore();
    await store.loadConversations();
    const count = await store.loadHistory('c1', 'c1', 10);
    expect(api.conversationMessages).toHaveBeenCalledWith('c1', { before: 10, limit: 50, channelId: 'c1' });
    expect(count).toBe(0);
  });
});
...
describe('handleFrame: read receipts', () => {
  it('advances my own read baseline, ignores other users’ read frames', async () => {
    ...
    await store.handleFrame({ type: 'read', conversationId: 'c1', channelId: 'c1', userId: 'friend', seq: 8 });
    expect(store.conversations[0]!.lastReadSeq).toBe(2); // other user → no change

    await store.handleFrame({ type: 'read', conversationId: 'c1', channelId: 'c1', userId: 'me', seq: 7 });
    expect(store.conversations[0]!.lastReadSeq).toBe(7); // my own read advances

    await store.handleFrame({ type: 'read', conversationId: 'c1', channelId: 'c1', userId: 'me', seq: 4 });
