...
- Default set of **custom emojis**: a few hundred popular emotes scraped from
  ...
  uploads (optimized as needed).
- **GIF search** via KLIPY (free tier), proxied server-side.
- **Integration API keys** (KLIPY etc.) live in a gitignored `.env`; see
  `.env.example` for the documented keys.
- Chat formatting: reuse the v2.1 live editor (code blocks, spoilers, colors) in
  the composer.
- Reactions, replies, and threads.
- Encrypted image/file attachments in chat (keyed by the conversation key).

**Deferred from v3.1:**

- **Link previews** — generated by the **sender's** client and embedded inside
  the encrypted message (Signal-style), so URLs never reach the server.
  *Deferred:* a browser PWA can't fetch arbitrary cross-origin pages (CORS), so
  the sender-side, server-never-sees-the-URL property isn't achievable in the
  browser. Revisit if a native/extension client (or an explicitly-accepted
  server-side proxy tradeoff) lands. See [v3.3](#v33--cleanup) backlog.
